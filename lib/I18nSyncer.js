  ...
  async push({ translationDir, sheetName, format = 'json', mainLanguage = 'en' } = {}) {
    try {
      ...
      if (!mainLanguageFile) {
        ...
        if (languageFiles.length > 0) {
          Object.keys(languageFiles[0].content).forEach(key => {
            if (!keySet.has(key)) {
              allKeys.push(key);
              keySet.add(key);
            }
      ...
      if (mainLanguageFile) {
        Object.keys(mainLanguageFile.content).forEach(key => {
          if (!keySet.has(key)) {
            allKeys.push(key);
            keySet.add(key);
          }
        });
        console.log(`Main language file (${mainLanguage}) has ${Object.keys(mainLanguageFile.content).length} keys`);
      }

      // Then add any additional keys from other language files
      languageFiles.forEach(({ langCode, content }) => {
